...
$templatelist = "forumbit_depth1_cat,forumbit_depth2_cat,forumbit_depth2_forum,forumbit_depth1_forum_lastpost,forumbit_depth2_forum_lastpost,forumbit_depth2_forum_unapproved_posts,forumbit_depth2_forum_unapproved_threads,forumdisplay_thread_icon,forumbit_depth2_forum_viewers";
$templatelist .= ",forumjump_advanced,forumjump_special,forumjump_bit,forumdisplay_password_wrongpass,forumdisplay_password,forumbit_subforums,forumbit_depth2_forum_lastpost_never,forumbit_depth2_forum_lastpost_hidden,forumbit_depth3_statusicon,forumbit_depth3";
...
$orderarrow = array('rating' => '', 'subject' => '', 'starter' => '', 'started' => '', 'replies' => '', 'views' => '', 'lastpost' => '');
...
if (!empty($threadCache) && is_array($threadCache)) {
    ...
    foreach ($threadCache as $key => $thread) {
        ...
        if ($thread['sticky'] == 1) {
            $thread['classType'] = "forumdisplay_sticky";
        } else {
            $thread['classType'] = "forumdisplay_regular";
        }
        ...
        if ($thread['lastposteruid'] > 0) {
            $thread['lastposter'] = build_profile_link(htmlspecialchars_uni($thread['lastposter']), $thread['lastposteruid']);
        }
        ...
        // Reassign this thread to the cache in order to let templates use the updated one
        $threadCache[$key] = $thread;
    }

    $customTools = [];
    if ($modpermissions['ismod']) {
        if ($modpermissions['canusecustomtools'] && $has_modtools == true) {
